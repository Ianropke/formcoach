...
export class CrossSetDriftAnalyzer {
  public static analyzeSession(sets: RecordedSet[]): WorkoutSessionAnalysis {
    ...
    if (romChangePct >= 10) {
      romTrend = 'degrading';
      performanceDriftIndex += Math.min(45, romChangePct * 2.2);
      observations.push({
        ...
        evidence: `Sæt 1 målte ${Math.round(firstROM)}° mod ${Math.round(lastROM)}° i sæt ${sets.length}.`,
        severity: 'warning',
        affectedReps: []
      });
    }
    ...
    if (tempoChangePct >= 18) {
      ...
      observations.push({
        ...
        evidence: `Gennemsnitlig varighed steg fra ${firstTempo.toFixed(1)}s til ${lastTempo.toFixed(1)}s.`,
        severity: 'info',
        affectedReps: []
      });
    }
    ...
    const index = Math.min(100, Math.round(performanceDriftIndex));
    return {
      totalSets: sets.length,
      totalReps,
      performanceDriftIndex: index,
      // Deprecated compatibility alias for older tests/callers. Do not use in new UI/domain code.
      fatigueIndex: index,
      romTrend,
      tempoTrend,
